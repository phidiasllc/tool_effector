...
module hotend_tool(
	...
	difference() {
		...
		// cut off the bottom of the effector
		translate([0, 0, -1.5 * h_effector_tool_mount])
			cylinder(r = 60, h = h_effector_tool_mount);
...
// holds a standard 5mm glass stirring rod
module mount_stir_rod() {
	circular_tool_mount(
		h_effector_tool_mount = 6.2,
		bearing_cage = true,
		d1_circular_tool = 5.5,
		clamping_screw = true,
		d_effector_tool_magnet_mount = d_small_effector_tool_magnet_mount);
}

module circular_tool_mount(
...
// the base from which distinct tool mounts can be fabricated
module effector_tool_mount(
	h_effector_tool_mount,
	bearing_cage = false,
	d_effector_tool_magnet_mount) {
	difference() {
		...
		if (bearing_cage)
			...
			for (i = [0:2])
				rotate([0, 0, i * 120 + 60])
					translate([0, d_effector_tool_magnet_mount / 2, 0])
						cylinder(r = d_M3_screw / 2, h = d_ball_bearing + 1, center = true);
	}
}
...
module tool_mount_body(
	...
	union() {
		...
		translate([5 * tan(30), d_effector_tool_magnet_mount * sin(30) / 2 + 5 * 2 * tan(30), 0])
			rotate([0, 0, 60])
...
module large_tool_effector() {
	difference() {
		union() {
			effector_tierod_magnets();

			cylinder(r = (d_large_effector_tool_mount + od_magnet) / 2 + r_pad_effector_magnet_mount + 2, h = t_effector, center = true);

			// put an index on the side normal the y-axis
			translate([5 * tan(30), (d_large_effector_tool_mount + od_magnet) / 2 + r_pad_effector_magnet_mount, 0])
				rotate([0, 0, 60])
					linear_extrude(height = t_effector, center = true)
						equilateral(5);
		}

		cylinder(r = (d_large_effector_tool_mount - od_magnet) / 2 - r_pad_effector_magnet_mount, h = t_effector + 1, center = true);
		...
		for (i = [0:2])
			rotate([0, 0, i * 120 + 60])
				for (j = [-1, 1])
				translate([0, d_large_effector_tool_mount / 2, j * t_effector / 2]) {
					cylinder(r = od_magnet / 2, h = 3, center = true);

					if (j < 0)
						translate([0, 0, 3 + layer_height])
							cylinder(r = d_M3_screw / 2, h = t_effector + 1);
				}
	}
}
